...
//클래스는 System.Serializablie 이라는 어트리뷰트(Attribute) 를 명시해야
//인스펙터 뷰에 노출됨
[System.Serializable]
...
public class PlayerCtrl : MonoBehaviour
{
    ...
    void Update()
    {
        h = Input.GetAxis("Horizontal");            //GetAxis("Horizontal") -> A,D 또는 화살표 키를 눌렀을때 -1.0부터 1.0까지의 값으로 반환 또는 누르지 않으면 0.0 반환
        v = Input.GetAxis("Vertical");              //GetAxis("Vertical") -> W,S 또는 화살표 키를 눌렀을 때 -1.0부터 1.0까지의 값으로 반환 또는 누르지 않으면 0.0 반환
        r = Input.GetAxis("Mouse X");


       // Debug.Log("h= " + h.ToString());
      //  Debug.Log("v= " + v.ToString());
      //  Debug.Log("PlayerCtrl.Update Call");
